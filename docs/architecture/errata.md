...
| E-PR18-1 | PR-18 | Split into **PR-18a** (API: authentication, endpoints, queries, OpenAPI, AT-01/AT-02 over HTTP) and **PR-18b** (`web/`: Next.js with types generated from OpenAPI). |
| E-PR18-2 | PR-18 | OIDC authorization code (PKCE) against Google Workspace, handled by the API; the `iam.session` id travels in an HttpOnly, Secure, SameSite=Strict cookie; every non-GET request also requires a custom header (anti-CSRF). Step-up = a new authorization with `prompt=login`, recorded by `SessionService.RecordStepUpAsync`. CI and development use a simulated IdP that exists only under `tests/`. |
| E-PR18-3 | PR-18 | Commands: `POST /api/v1/companies/{companyId}/{module}/{command}` with the command body; the `Idempotency-Key` header is required (a duplicate returns the same result, flagged as replayed). Errors are RFC 9457 problem+json with the domain code: no session 401; no permission or step-up needed 403; version change or conflict 409; business rule 422. |
| E-PR18-4 | PR-18 | New READ permissions `purchase_order:read`, `goods_receipt:read`, `supplier_invoice:read`, `period:read`, `master_data:read`, granted to the roles that work those documents (plus Controller and Auditor); the list and detail queries run on the PR-17 query pipeline. |
...
| E-PR19-1 | PR-19 | Every acceptance test of §15 and Patch 1 is tagged `[Trait("Acceptance", "<ID>")]`; an architecture test fails when a baseline ID has no tagged test; the matrix lives in `docs/acceptance/vs1.md`. |
| E-PR19-2 | PR-19 | CC-04: 50 concurrent workers for 60 s over 3 POs and 2 items (receipts, corrections created and approved, reversals, supplier invoices registered/matched/posted, fixture issues). Passes with no FAILED_TECHNICAL and no deadlock left after retries, only expected business rejections, and AT-04, INV-QTY-BALANCE, INV-VALUE-GL and VALUE-GL-LINK MATCHED. Runs in the regular CI. |
| E-PR19-3 | PR-19 | PF-01: console harness `tests/Rochell.LoadHarness`; 10,000 goods receipts through the real command pipeline (no HTTP), 8 workers, sealer running; p95 of PostGoodsReceipt < 500 ms and the 8 reconciliations together < 30 s. Separate workflow (manual and weekly) with a report artifact; reference hardware is the GitHub `ubuntu-24.04` runner until staging exists (B-03), where it is repeated as an acceptance condition. |
| E-PR19-4 | PR-19 | P-7 `definition_hash` is satisfied by immutable fiscal definitions (E-PR12-4): a definition can never change and every version needs its own passing test run. FIS-01 reads: modifying a definition is rejected; a new version is not activated without its own passing run. |
| E-PR19-5 | PR-19 | VS#1 does not model availability (formalizes E-PR07-7): IV-04 is a schema test that no RESERVED value and no `qty_reserved` column exist. |
| E-PR19-6 | PR-19 | `physical_lot_issue_policy` (E-7) is deferred until real issues exist (production); IV-05 is verified as the invariant INV-09: issuing from lot A or lot B, received at different costs, gives the same accounting amounts. |
| E-PR19-7 | PR-19 | Partial acceptance tests are completed with tests only (CC-02, FIS-02, FIS-03, INT-02, INT-03, IV-01, RC-01b, RO-02, SI-06b, VAL-01, VAL-02, VAL-03); the one behaviour change: the hosted sealer logs SEAL_ERROR at Critical (INT-02's critical alert). |
| E-PR19-8 | PR-19 | `docs/acceptance/vs1.md` holds the matrix, CI links, the PF-01 result and the open conditions; the slice is declared accepted only after the second-person ledger review (B-02); B-03 and A-01…A-03 remain conditions. |
| E-PR19-9 | PR-19 | P-7 as written: `tax.fiscal_rule_source.environment` (TEST/PRODUCTION, stated explicitly by `RegisterFiscalSource`; rows that already exist, only in test databases, become TEST); in PRODUCTION the activation gate requires at least one linked PRODUCTION source; the gate reads the environment through `core.current_environment()` (Patch 1.1). |
| E-PR19-10 | PR-19 | Precision 1 (subledger = GL by transaction deltas) was decided by PF-01: with the full-position check the reference runner measured p95 502 ms (limit 500), so the delta form is implemented (migration 0021): each ledger insert adds its delta to control totals on the valuation row through SECURITY DEFINER triggers, and the deferred check compares that row (O(1)). Local p95 went from 287 to 60 ms and no longer grows with history. Ledger review B-02 covers it. |
| E-PR19-11 | PR-19 | Nightly reconciliations (ACC-EVIDENCE, INV-VALUE-GL) are not scheduled in VS#1 (commands need a human session; no service identities yet): they run on demand and CloseComponent runs the blocking ones in its transaction. Scheduling is an operational condition listed in `docs/acceptance/vs1.md`. |
| E-PR19-12 | PR-19 | VS#1 has a single owner, the company: inventory ledgers carry no `accounting_owner` / `legal_title_holder` (formalizes E-PR07-7); separate ownership (consignment, third parties) is deferred; a schema test documents it. |
...
- The API host adds transport only (E-PR18-1): the command route's module is the assembly that owns the handler and the command segment its type name in kebab-case; server-filled fields (`companyId`, `sessionId`, `idempotencyKey`) are refused in the body.
- The session cookie value is the `iam.session` id protected with ASP.NET Data Protection, so an id read from `command_log` or `domain_event` is not a credential; step-up carries the protected id in the OIDC state because the SameSite=Strict cookie is not sent on the cross-site callback.
- On the read side a missing document is 404 (`NOT_FOUND`) and a bad filter or paging value 400 (`INVALID_PARAMETER`); exhausted serialization retries are 409 `CONCURRENCY_CONFLICT`; hash verification without WORM storage is 503.
