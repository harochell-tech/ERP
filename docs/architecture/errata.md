...
| E-PR19-10 | PR-19 | Precision 1 (subledger = GL by transaction deltas) is decided by PF-01: measured with the current full-position check first; if p95 < 500 ms the full check stays (it verifies total equality at every COMMIT) and replaces precision 1; otherwise the delta form is implemented (ledger review B-02). |
