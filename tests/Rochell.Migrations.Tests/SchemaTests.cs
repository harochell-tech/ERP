...
public sealed class SchemaTests(PostgresFixture postgres)
{
    ...
    public async Task No_tables_outside_authorized_prs_exist()
    {
        ...
        // PR-01: md.company. PR-02: core.*, obs.request_log. PR-03: iam.*. PR-04: md masters. PR-05: fin.*. PR-06: acc.*. PR-07: inv.*. PR-08: pur.purchase_order*. PR-09: pur.goods_receipt*.
        Assert.Equal(
            ...
            + "pur.goods_receipt,pur.goods_receipt_line,pur.purchase_order,pur.purchase_order_line",
            tables);
    }
}
